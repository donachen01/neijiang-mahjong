...
Usage:
  Blender --background --python tools/3d/generate_neijiang_ui_shells_v2.py
"""
...
def render_hud_shell() -> None:
    ...
    copper = material("HUDAgedGold", "A8772E", 0.48, 0.38)
    highlight = material("HUDGoldHighlight", "E4BD62", 0.38, 0.34)
    ivory = material("HUDWarmIvory", "F1DDAF", 0.03, 0.48)
    inner = material("HUDPearlIvory", "FFF4D8", 0.01, 0.56)
    # Runtime HUD is a compact 310x118 nameplate (2.63:1). Author the shell at
    # the same aspect ratio so the nine-patch never folds its outer shadow into
    # a horizontal bar through the player name.
    cut_corner_prism("CopperOuter", 9.18, 3.22, 0.28, 0.18, 0.00, copper, 0.09)
    cut_corner_prism("IvoryBody", 9.00, 3.04, 0.25, 0.18, 0.13, ivory, 0.08)
    cut_corner_prism("PearlInset", 8.62, 2.66, 0.20, 0.10, 0.25, inner, 0.055)
    # Keep the text field intentionally clean. Long embossed rails compressed into the
    # short runtime nine-patch previously crossed player names like a dark strike-through.
    add_camera_and_lights(ortho_scale=10.00)
    output = OUTPUT_DIR / "hud_shell.png"
    configure_render(960, 360, output)
...
def main() -> None:
    ...
    if "--hud-only" in sys.argv:
        render_hud_shell()
        print(f"Generated bright Neijiang HUD shell in {OUTPUT_DIR}")
        return
