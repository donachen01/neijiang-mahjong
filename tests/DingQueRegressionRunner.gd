...
func _ready() -> void:
	...
	_run_test("neijiang_ai_bao_jiao_discard_falls_back_to_last_draw", _test_neijiang_ai_bao_jiao_discard_falls_back_to_last_draw, failures)
	...
	var players: Array[Dictionary] = [
		...
		_make_player(
			...
			[
				...
				_make_tile(145, "wan", 8), _make_tile(146, "wan", 8),
			]
		),
	]
	game_state.players = players
...
func _test_neijiang_bao_jiao_plan_is_available_on_opening_ting():
	...
	if not bool(game_state.can_human_bao_jiao(0)):
		return "expected opening hand to allow bao jiao"
...
func _test_neijiang_bao_gang_whitelist_requires_keep_ting():
	...
	if not keys.has("tong_8"):
		return "expected four-of-a-kind tong 8 to remain bao gang eligible"
	if keys.has("tong_9"):
...
func _test_neijiang_ai_bao_jiao_discard_falls_back_to_last_draw():
	var game_state = _build_neijiang_test_game_state()
	game_state.current_phase = GAME_STATE_SCRIPT.RoundPhase.DISCARD
	...
	if not executed:
		return "expected AI bao-jiao fallback to discard the just-drawn tile, got %s" % game_state.debug_last_message
...
func _build_neijiang_test_game_state():
	var game_state = _build_test_game_state()
	game_state.rules.apply_mode(load("res://scripts/core/rule_config.gd").MODE_NEIJIANG_CLASSIC)
	return game_state
