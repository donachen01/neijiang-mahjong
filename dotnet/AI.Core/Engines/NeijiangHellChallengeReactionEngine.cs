...
public sealed class NeijiangHellChallengeReactionEngine
{
    private readonly NeijiangReactionDecisionEngine _fair = new();
        ...
        bool mandatoryGang = false)
    {
        ...
        if (state.IsBaoJiao)
            return fair;
        if (mandatoryGang && fair.Action.ActionType == NeijiangActionType.Gang)
            return fair;
    ...
    private NeijiangReactionDecisionResult BuildPassResult(
        int reactionTileType,
        int currentShanten,
        int currentLive,
        int humanPressure,
        ...
        Dictionary<string, int> inheritedScores)
    {
        ...
        var gangBonus = actionType == NeijiangActionType.Gang ? 160 : 0;
        var nonHumanMiddlePengPenalty = 0;
        if (actionType == NeijiangActionType.Peng
            && !blocksHuman
            && shantenAfter > 0
        ...
        if (actionType == NeijiangActionType.Gang)
    ...
    private NeijiangReactionDecisionResult ApplyPengRediscardSameTilePenalty(
        ...
        IReadOnlyList<int> exactWall18)
    {
        if (reactionTileType is < 0 or >= 18 || handAfterPeng[reactionTileType] <= 0)
            return peng;
    ...
    private int EstimateBestFollowUpDiscard(
        ...
        IReadOnlyList<int> exactWall18)
    {
        var postPengState = new NeijiangStateView
        {
    ...
    private NeijiangReactionDecisionResult BuildResult(
        NeijiangActionType actionType,
        int reactionTileType,
        int score,
        int currentShanten,
        ...
        => new()
        {
            Action = new NeijiangAction(actionType, reactionTileType, score, reasons.FirstOrDefault() ?? ""),
