...
public sealed class NeijiangHandEvaluator
{
    ...
    private static int EstimateBigHandPotential(NeijiangStateView state, int pairCount)
    {
        var suitCounts = new[] { 0, 0 };
        for (var tile = 0; tile < 18; tile++)
    ...
    private static int CountHighRiskWasteTiles(NeijiangStateView state, NeijiangBeliefSnapshot belief)
    {
        var count = 0;
        for (var tile = 0; tile < 18; tile++)
        {
...
{
    public (NeijiangScoreSituation Score, NeijiangRiskTolerance Risk, NeijiangRoundGoalContext Goal) Evaluate(NeijiangStateView state)
    {
...
public sealed class NeijiangOpponentDangerEvaluator
{
    public IReadOnlyDictionary<int, NeijiangOpponentDangerProfile> Evaluate(
        NeijiangStateView state,
        NeijiangBeliefSnapshot belief,
        NeijiangStageContext stage)
    {
        var result = new Dictionary<int, NeijiangOpponentDangerProfile>();
    ...
    private static int EstimateBigHandRisk(NeijiangStateView state, int seat)
    {
...
public sealed class NeijiangTileDangerEvaluator
{
    private readonly NeijiangDangerEngine _danger = new();

    public IReadOnlyDictionary<int, NeijiangTileDangerProfile> Evaluate(
        NeijiangStateView state,
        NeijiangBeliefSnapshot belief,
        IReadOnlyDictionary<int, NeijiangOpponentDangerProfile> opponents,
        NeijiangStageContext stage)
    {
        var result = new Dictionary<int, NeijiangTileDangerProfile>();
        for (var tile = 0; tile < 18; tile++)
        {
            var global = _danger.EvaluateDetail(tile, state, belief);
            var byOpponent = new Dictionary<int, NeijiangSeatTileDanger>();
            foreach (var (seat, profile) in opponents)
            {
                var score = EstimateSeatTileDanger(state, belief, seat, tile, profile, stage);
                byOpponent[seat] = new NeijiangSeatTileDanger
                {
                    Seat = seat,
                    Score = score,
    ...
    private static int EstimateSeatTileDanger(
        ...
        int seat,
        int tile,
        NeijiangOpponentDangerProfile profile,
        NeijiangStageContext stage)
    {
    ...
    private static IReadOnlyList<string> ConvertRiskReasons(IReadOnlyList<string> reasons, int risk, NeijiangStageContext stage)
    {
...
public sealed class NeijiangAttackEligibilityEvaluator
{
    public NeijiangAttackEligibility Evaluate(
        NeijiangHandAnalysis hand,
        NeijiangStageContext stage,
        IReadOnlyDictionary<int, NeijiangOpponentDangerProfile> opponents,
        NeijiangScoreSituation scoreSituation,
        NeijiangRiskTolerance riskTolerance)
    {
    ...
    private static NeijiangAttackEligibility Build(string level, string reasonCode, int score) => new()
    {
        Level = level,
        ReasonCode = reasonCode,
...
public sealed class NeijiangStrategyModeStateMachine
{
    public NeijiangStrategyModeContext Evaluate(
        string previousMode,
        NeijiangRoundGoalContext roundGoal,
        NeijiangAttackEligibility eligibility,
        NeijiangStageContext stage,
        NeijiangScoreSituation scoreSituation,
        IReadOnlyDictionary<int, NeijiangOpponentDangerProfile> opponents)
    {
        ...
        return new NeijiangStrategyModeContext
        {
            Mode = mode,
...
public sealed class NeijiangDealInPolicyEvaluator
{
    public NeijiangDealInPolicy Evaluate(int tileType, NeijiangAiContext context)
    {
        ...
        if (maxDanger >= 82 && bigHandRisk >= 65)
            return new NeijiangDealInPolicy
            {
                TileType = tileType,
                AdjustmentScore = -900,
