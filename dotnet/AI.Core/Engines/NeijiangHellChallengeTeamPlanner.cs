...
public static class NeijiangHellChallengeTeamPlanner
{
    public static NeijiangHellChallengeTeamPlan BuildPlan(
    ...
    {
        var humanPressure = NeijiangHellChallengeEngine.ResolveHumanPressureLevel(state, currentScores);
        var targetSeat = 0;
        var bestAiSeat = ResolveBestAiSeat(currentScores, fallbackSeat: state.SeatIndex);
        var orderedAiSeats = new[] { 1, 2, 3 };
        var plans = new List<NeijiangHellChallengeSeatPlan>();
        ...
        foreach (var seat in orderedAiSeats)
        {
            ...
            plans.Add(new NeijiangHellChallengeSeatPlan
            {
                Seat = seat,
                Role = role,
        ...
        return new NeijiangHellChallengeTeamPlan
        {
            ...
            Reasons = new[]
            {
                ...
                $"三家协作：牌墙透视 {exactWall18.Take(18).Sum()} 张"
            }
        };
    }
