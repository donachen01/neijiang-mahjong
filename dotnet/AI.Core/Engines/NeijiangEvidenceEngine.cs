...
public sealed class NeijiangEvidenceEngine
{
    public NeijiangEvidenceSnapshot Build(NeijiangStateView state)
    {
        var snapshot = new NeijiangEvidenceSnapshot();
        for (var seat = 0; seat < 4; seat++)
        {
            var discards = state.Discards18[seat];
            var discardByTile = new int[18];
            var discardBySuit = new int[2];
    ...
    {
        if (seat < 0 || seat >= countsBySeat.Length || tileType is < 0 or >= 18)
            return 0.0;
