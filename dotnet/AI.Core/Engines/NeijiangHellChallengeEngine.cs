...
public sealed class NeijiangHellChallengeEngine
{
        ...
        IReadOnlyList<int>? currentScores = null)
    {
        ...
        if (baoJiaoDecision is not null)
        {
            ...
            return new NeijiangHellOracleResult
            {
                ...
                Action = baoJiaoDecision.Action,
                Category = "bao_jiao_route",
                Severity = "none",
    ...
    public static IReadOnlyList<int> ResolveDealInTargetSeats(NeijiangStateView state, IReadOnlyList<IReadOnlyList<int>> allHands18, int discardTileType)
    {
    ...
    public static bool CanHumanPeng(NeijiangStateView state, IReadOnlyList<IReadOnlyList<int>> allHands18, int discardTileType)
        => CanHumanCall(state, allHands18, discardTileType, 2);

    public static bool CanHumanGang(NeijiangStateView state, IReadOnlyList<IReadOnlyList<int>> allHands18, int discardTileType)
    ...
    private int EstimateHumanPengThreat(
        ...
        int discardTileType)
    {
        if (state.SeatIndex == 0 || discardTileType is < 0 or >= 18 || allHands18.Count <= 0 || allHands18[0].Count <= discardTileType)
            return 0;
        var humanHand = allHands18[0].Take(18).Concat(Enumerable.Repeat(0, 18)).Take(18).ToArray();
        ...
        if (concealedCount > 1)
        {
            var wall = exactWall18.Take(18).Concat(Enumerable.Repeat(0, 18)).Take(18).ToArray();
            for (var tileType = 0; tileType < 18; tileType++)
            {
    ...
    private static int CalculateHumanPengPenalty(
        NeijiangHellChallengeSeatPlan seatPlan,
    ...
    private static int ResolveSpeedPressure(int wallCount, int shanten, bool keepsReady)
    {
        ...
        if (shanten == 2 && wallCount >= 18)
            return 120;
        return 0;
    }

    public static int ResolveHumanPressureLevel(NeijiangStateView state, IReadOnlyList<int>? currentScores)
    {
