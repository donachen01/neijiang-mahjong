...
public sealed class NeijiangWaitShapeEngine
{
    public NeijiangWaitShapeSummary Evaluate(int[] hand18AfterDiscard, IReadOnlyList<int> waitTiles)
    {
        if (waitTiles.Count == 0)
        {
            return new NeijiangWaitShapeSummary
            {
                Label = "未成听",
                Reasons = new[] { "未成听形" }
            };
