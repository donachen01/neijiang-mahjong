...
public sealed class NeijiangOpponentRangeEngine
{
    public NeijiangOpponentRangeProfile BuildSeatRange(
        ...
        int seat)
    {
        ...
        foreach (var tile in discards.Where(tile => tile is >= 0 and < 18))
            discardBySuit[tile / 9]++;
        foreach (var tile in meldTiles.Where(tile => tile is >= 0 and < 18))
            meldBySuit[tile / 9]++;

        var suitDemand = new double[2];
        ...
        return new NeijiangOpponentRangeProfile
        {
            Seat = seat,
    ...
    private static double EstimateReadyProbability(NeijiangStateView state, int seat, int discardCount, int meldGroupCount, double pressure)
    {
    ...
    private static double[] BuildWallPosterior(NeijiangStateView state, double[] holdProbability, int seat)
    {
        var wallPosterior = new double[18];
        for (var tileType = 0; tileType < 18; tileType++)
        {
