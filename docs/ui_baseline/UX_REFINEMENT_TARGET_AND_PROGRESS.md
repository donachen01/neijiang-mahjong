...
| 桌面统一性 | 无明显分区底色/边框；桌面有连续绒布纹理、中心光、暗角，但无抢眼装饰线 | 84% |
| 麻将牌本体 | 厚牌圆角、暖象牙牌面、绿色侧边、统一投影；正背面同一资产体系 | 83% |
| 正面牌纹 | 条、筒、万、幺鸡等符号与新牌底同风格，线条柔和清晰，不像旧贴图 | 76% |
| 牌背 | 素玉牌背与桌面有足够分离度，无旧圈纹，阴影统一 | 84% |
| 玩家信息 HUD | 贴边、清晰、弱面板化；字体、头像/定缺、分数、已胡状态统一 | 78% |
| 操作按钮 | 右上/动作按钮采用同系列 3D 果冻/玉石按钮，触控目标清晰 | 77% |
| AI 提示 | 提示框居中、大字、只显示内容；推荐牌用悬浮 3D 标记，不遮挡读牌 | 74% |
| 可访问性 | 手机横屏玩家名/分数/状态可读；关键文字对比度足够，不仅靠颜色表达状态 | 78% |
| 启动图一致性 | 整体材质、光照、圆润度、颜色气质接近启动界面 | 78% |
...
| 1.0.23 | 本次提交 | 2026-05-11 | 桌面与牌体资产微调：中心暖光更贴近启动图气质，边缘框线进一步柔化；牌背绿色提亮并保留暗边，牌面侧边和底部阴影更温润；同步版本号 | `V17 LAYOUT CONTRACT OK`；截图 `docs/ui_baseline/captures/main_scene_v1_0_23_candidate.png`、`docs/ui_baseline/captures/main_scene_v1_0_23_mobile_1365x768.png` | 94% | 继续步骤 5/7/8：AI 提示和最终移动端验收，准备 100% 闭环与最终打包 |

## 6. 未完成任务池

- 继续精修整套正面牌纹资源；1.0.21 已给幺鸡与万字增加单牌覆盖和暗线染色，下一轮只做截图评审级微调，避免继续过度加工。
- 继续将本家手牌从“白片感”提升为明显厚玉牌；1.0.23 已重生成更温润的牌面/牌背资产，下一轮只做最终截图验收微调。
- 统一桌面小牌、碰杠牌、胡牌、弃牌的材质比例；1.0.23 已提高牌背与桌面的分离度，下一轮继续检查 AI 提示叠加时的远景可读性。
- 继续弱化玩家信息框旧式大面板感；1.0.22 已统一报叫/杠/定缺状态标记边框和非颜色提示，下一轮只做最终截图评审级微调。
- 继续重做右上角按钮和动作按钮为同系列 3D 控件；1.0.20 已补程序化贴图级高光，下一轮需评估是否进一步图标化，避免增加认知负担。
- AI 推荐已从旋转锥体改为更轻的玉金悬浮标记，并放入牌面上沿安全区；下一轮继续检查动画层级和与危险/胡牌标记的区分度。
- 继续减少桌面边缘像边框的装饰线，保留绒布暗角和中心光；1.0.23 已进一步柔化边缘线并暖化中心光，下一轮以最终截图评审为准。
