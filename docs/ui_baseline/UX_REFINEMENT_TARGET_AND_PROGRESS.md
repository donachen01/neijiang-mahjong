...
| 玩家信息 HUD | 贴边、清晰、弱面板化；字体、头像/定缺、分数、已胡状态统一 | 75% |
| 操作按钮 | 右上/动作按钮采用同系列 3D 果冻/玉石按钮，触控目标清晰 | 77% |
| AI 提示 | 提示框居中、大字、只显示内容；推荐牌用悬浮 3D 标记，不遮挡读牌 | 74% |
| 可访问性 | 手机横屏玩家名/分数/状态可读；关键文字对比度足够，不仅靠颜色表达状态 | 75% |
...
- 继续弱化玩家信息框旧式大面板感；1.0.19 已提升移动端侧边身份 HUD 字号和可读区，1.0.20 已让按钮系统共享同源高光层，下一轮重点做 HUD 状态标记统一。
