...
const NeijiangDecisionEngineScript := preload("res://scripts/ai/neijiang_decision_engine.gd")
...
func _evaluate_peng(
	...
	var strategy_mode := str(shape.get("mode_label", "两门速听"))
	...
	if is_neijiang and best_shanten <= 0:
		score += 160
		reasons.append("碰后可直接成叫，内江优先尽快听牌")
...
func _evaluate_pass(player: Dictionary, players: Array, current_shape: Dictionary) -> Dictionary:
	...
	var strategy_mode := str(current_shape.get("mode_label", "两门速听"))
	if strategy_mode in ["全守", "防守平衡"]:
		score += 42
		reasons.append("当前偏防守，过更稳妥")
	elif strategy_mode == "两门速听":
		score += 12
