...
const MODE_NEIJIANG_CLASSIC := "neijiang_classic"

const ALL_SUITS := ["tiao", "tong", "wan"]
const NEIJIANG_SUITS := ["tiao", "tong"]
...
func apply_mode(mode_name: String) -> void:
	mode = mode_name
	match mode_name:
		MODE_NEIJIANG_CLASSIC:
			_apply_neijiang_defaults()
		_:
			_apply_sichuan_defaults()


func is_neijiang_mode() -> bool:
	return mode == MODE_NEIJIANG_CLASSIC
...
func _apply_neijiang_defaults() -> void:
	available_suits = NEIJIANG_SUITS.duplicate()
	total_tile_count = 72
	allow_chi = false
	allow_peng = true
	allow_gang = true
	...
	allow_multi_win_on_discard = true
	enable_qi_dui = true
	require_missing_one_suit_to_win = false
	use_battle_to_end_flow = true
	max_winners_per_round = 3
	fan_cap = 5
	use_ding_que_phase = false
	enable_bao_jiao = true
	enable_bao_gang = true
	enable_ka_er_tiao = true
	enable_gui = true
	enable_cha_jiao = true
	enable_tui_shui = true
