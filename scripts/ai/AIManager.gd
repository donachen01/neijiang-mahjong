...
const CSHARP_DLL_PATH := "res://dotnet/AI.Core/bin/Release/net10.0/NeijiangMahjong.AI.Core.dll"
...
func analyze_turn_lightweight(player_state: Dictionary, table_state: Dictionary, rules_config, ai_config, hu_checker, risk_analyzer, allow_cheat: bool = false) -> Dictionary:
	var result := _compute_turn_analysis(player_state, table_state, rules_config, ai_config, hu_checker, risk_analyzer, allow_cheat, "lightweight", false)
	_finalize_turn_analysis(int(player_state.get("seat", -1)), result)
	return result.get("analysis", {}).duplicate(true)
...
func analyze_self_action(player_state: Dictionary, table_state: Dictionary, rules_config, can_self_hu: bool, an_gang_tile_types: Array, add_gang_tile_types: Array, add_gang_qiang_gang_counts: Dictionary = {}) -> Dictionary:
	var started_at_ms := Time.get_ticks_msec()
	var analysis: Dictionary = {}
	var active_backend := "csharp_self_action_required"
	last_native_turn_error = ""
	if has_native_csharp_runtime() and rules_config != null and bool(rules_config.is_neijiang_mode()):
		var payload: Dictionary = csharp_bridge.build_self_action_transport_payload(player_state, table_state, rules_config, can_self_hu, an_gang_tile_types, add_gang_tile_types, add_gang_qiang_gang_counts)
		...
		var csharp_result := csharp_bridge.analyze_self_action(player_state, table_state, rules_config, can_self_hu, an_gang_tile_types, add_gang_tile_types, add_gang_qiang_gang_counts, "self_action")
	...
	if analysis.is_empty() and rules_config != null and bool(rules_config.is_neijiang_mode()):
...
func _analyze_discard_via_native_runtime(player_state: Dictionary, table_state: Dictionary, rules_config) -> Dictionary:
	if not has_native_csharp_runtime():
	...
	last_native_turn_raw_summary = "payload seat=%d hand_sum=%d wall=%d" % [
...
func _build_hybrid_option(csharp_item: Dictionary, support_option: Dictionary, active_suits: Array, bridge_instance = null) -> Dictionary:
	...
	option["strategy_mode"] = str(csharp_item.get("strategyMode", option.get("strategy_mode", "两门速听")))
...
func _merge_strategy_profile(base_profile: Dictionary, csharp_profile: Dictionary) -> Dictionary:
	...
	merged["mode_label"] = str(csharp_profile.get("mode_label", merged.get("mode_label", "两门速听")))
...
func _compute_turn_analysis(player_state: Dictionary, table_state: Dictionary, rules_config, ai_config, hu_checker, risk_analyzer, allow_cheat: bool, request_tag: String, force_gdscript: bool = false) -> Dictionary:
	var cache_key := _build_turn_cache_key(player_state, table_state, rules_config, allow_cheat, force_gdscript)
	...
	last_native_turn_error = ""
	if not force_gdscript and has_native_csharp_runtime() and rules_config != null and bool(rules_config.is_neijiang_mode()):
	...
	elif analysis.is_empty() and (rules_config == null or not bool(rules_config.is_neijiang_mode())):
		analysis = local_bridge.request_discard(payload, hu_checker, risk_analyzer)
	var elapsed_ms := maxi(0, Time.get_ticks_msec() - started_at_ms)
	var budget_ms := _resolve_budget_ms("turn", active_backend)
	var result := {
		"analysis": analysis.duplicate(true),
...
func _compute_reaction_analysis(candidate: Dictionary, player_state: Dictionary, table_state: Dictionary, discard_context: Dictionary, rules_config, ai_config, hu_checker, allow_cheat: bool) -> Dictionary:
	...
	if analysis.is_empty() and rules_config != null and bool(rules_config.is_neijiang_mode()) and strict_native_runtime_required:
		...
		analysis = local_bridge.request_reaction(payload, hu_checker)
	var elapsed_ms := maxi(0, Time.get_ticks_msec() - started_at_ms)
	var budget_ms := _resolve_budget_ms("reaction", active_backend)
	return {
		"analysis": analysis.duplicate(true),
...
func _thread_compute_turn(request_id: int, player_state: Dictionary, table_state: Dictionary, rules_config, ai_config, hu_checker, risk_analyzer, allow_cheat: bool) -> Dictionary:
	return {
		"request_id": request_id,
		"kind": "turn",
		"seat": int(player_state.get("seat", -1)),
		"result": _compute_turn_analysis(player_state, table_state, rules_config, ai_config, hu_checker, risk_analyzer, allow_cheat, "bg_%d" % request_id),
	}
...
func _build_turn_cache_key(player_state: Dictionary, table_state: Dictionary, rules_config, allow_cheat: bool, force_gdscript: bool) -> String:
