...
class_name NeijiangDecisionEngine

const ShantenEngineScript := preload("res://scripts/ai/neijiang_shanten_engine.gd")
const DangerEngineScript := preload("res://scripts/ai/neijiang_danger_engine.gd")
const BeliefEngineScript := preload("res://scripts/ai/neijiang_belief_engine.gd")
const CallQualityEngineScript := preload("res://scripts/ai/neijiang_call_quality_engine.gd")
const TileCodecScript := preload("res://scripts/ai/neijiang_tile_codec.gd")
...
func build_support_context(player: Dictionary, players: Array, rules_config, hu_checker, risk_analyzer, ai_config = null, allow_cheat: bool = false) -> Dictionary:
	...
	return {
		...
		"strategy_profile": {
			...
			"reasons": ["两门牌优先追求最小向听、宽叫与活张", "当前牌形 %s" % str(two_suit_state.get("state_label", "两门均衡"))],
		},
