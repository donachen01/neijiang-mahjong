class_name NeijiangSeatAvatarMedallion
extends Control

const COPPER_SHADOW := Color("5A4027")
...
func _draw() -> void:
	...
	# 四川版同款玉石头像章：静态多层圆面模拟浅浮雕，避免额外纹理采样。
	draw_circle(center + Vector2(3.0, 5.0), radius + 2.0, Color(0.0, 0.015, 0.01, 0.46))
	draw_circle(center, radius + 2.0, Color(COPPER_SHADOW, 0.92))
	...
	if active:
		draw_arc(center, radius + 3.0, 0.0, TAU, 56, Color(COPPER_HIGHLIGHT, 0.98), 3.0, true)
		draw_arc(center, radius + 6.0, -PI * 0.18, PI * 0.64, 28, Color(1.0, 0.90, 0.48, 0.74), 2.0, true)
	else:
		draw_arc(center, radius + 3.0, 0.0, TAU, 56, Color(AGED_COPPER, 0.42), 1.5, true)
	if won:
		draw_circle(center, radius - 1.0, Color(0.02, 0.035, 0.03, 0.44))
		var stamp_center := center + Vector2(radius * 0.52, radius * 0.48)
		var stamp_radius := radius * 0.24
		draw_circle(stamp_center, stamp_radius, Color(CINNABAR, 0.96))
		draw_arc(stamp_center, stamp_radius, 0.0, TAU, 24, Color(COPPER_HIGHLIGHT, 0.88), 1.5, true)
