class_name NeijiangTableSkinPanel
extends Control
...
func _apply_focus_navigation() -> void:
	...
	for index in range(ordered.size()):
		...
		var column := index % 2
		var left_index := row * 2 + (1 - column)
		var up_index := maxi(0, index - 2)
		var down_index := mini(ordered.size() - 1, index + 2)
		ordered[index].focus_neighbor_left = ordered[index].get_path_to(ordered[left_index])
		ordered[index].focus_neighbor_right = ordered[index].get_path_to(ordered[left_index])
