...
func apply_hint(trainer_hint: Dictionary, can_discard: bool, selected_tile_id: int = -1) -> void:
	...
	if selected_tile_id != -1 and selected_tile_id != recommended_tile_id:
		...
		if not selected_option.is_empty():
			summary_label.text += " | 当前选中 %s 向听%d 进张%d 胡率%.1f%% 风险%s" % [
				str(selected_option.get("tile_name", "?")),
				int(selected_option.get("shanten", 8)),
				int(selected_option.get("ukeire", 0)),
				float(selected_option.get("win_probability", 0.0)) * 100.0,
				str(selected_option.get("risk_label", "低危")),
			]
	...
	if not reasons.is_empty():
		reason_parts.append("基础：%s" % "；".join(reasons.slice(0, 2)))
	if not posterior_reasons.is_empty() and str(posterior_reasons[0]) != "后验未明显压分":
		reason_parts.append("后验：%s" % "；".join(posterior_reasons.slice(0, 2)))
	if reason_parts.is_empty():
		reason_parts.append("当前以效率和防守平衡为主")
	reason_label.text = "理由：%s" % "\n".join(reason_parts)
	...
	danger_label.text = "危险张：%s" % (" / ".join(danger_lines) if not danger_lines.is_empty() else "暂无明显危险张")

	var route_text := "/".join(trainer_hint.get("current_routes", []))
	if route_text.is_empty():
		route_text = "当前以效率和防守为主"
	routes_label.text = "可做牌型：%s" % route_text
...
func _build_selected_option_delta_text(recommended: Dictionary, selected_option: Dictionary) -> String:
	...
	if delta_shanten > 0:
		...
		parts.append("向听更快 %d" % abs(delta_shanten))
	if delta_ukeire < 0:
		parts.append("活进张少 %d" % abs(delta_ukeire))
	elif delta_ukeire > 0:
		parts.append("活进张多 %d" % delta_ukeire)
	if delta_risk > 0:
