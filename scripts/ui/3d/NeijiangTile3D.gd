...
# 以用户提供的四川成品所对应的 2.6.9 玉石牌合同为准。暖白承托层只负责
# 统一本家竖牌正面白度；圆角、侧壁、绿背和接触阴影仍由 Blender 实体承担。
const SELF_HAND_FACE_WHITE := Color("FFF8E8")
...
static func get_manufacturing_contract() -> Dictionary:
	# Blender 实体是两层；正面承托、字图和状态标记是 Godot 运行时层。
	# 数值可以让后续渲染调整有一个可回归的“制造合同”，避免再靠截图猜测。
	return {
		"blender_version": "5.2.39",
		"physical_layer_count": 2,
		"total_size": Vector3(0.420, 0.240, 0.580),
		"layers": [
			{
				"name": "MahjongTileBack",
				"size": Vector3(0.414, 0.074, 0.574),
				"bevel": 0.022,
				"bevel_segments": 6,
				"color": NORMAL_TILE_BACK_COLOR.to_html(false),
				"roughness": 0.27,
				"metallic": 0.02,
				"clearcoat": 0.34,
			},
			{
				...
				"size": Vector3(0.420, 0.166, 0.580),
				"bevel": 0.032,
				"bevel_segments": 8,
				"color": "e4ded2",
				"bright_color": "e8e1d2",
		...
		"runtime_layers": [
			{"name": "FaceCarrier", "height": FACE_Y, "size": FACE_SIZE, "color": SELF_HAND_FACE_WHITE.to_html(false)},
			{"name": "SymbolPrint", "height": FACE_Y + 0.003, "size": FACE_SIZE},
			{"name": "OptionalStateMarker", "height": MARKER_Y},
		],
	}
...
func configure(
	...
	_apply_state_marker(selected, new_draw, recommended, danger, bao_gang_declared)
...
func _face_material(show_face: bool) -> StandardMaterial3D:
	...
	if show_face:
		...
		result.albedo_color = Color("E4DED2")
		...
		result.clearcoat = 0.28
		result.clearcoat_roughness = 0.24
...
func _bright_front_face_material() -> StandardMaterial3D:
	const CACHE_KEY := "face:self_hand_sichuan_2_6_9_v1"
	if material_cache.has(CACHE_KEY):
		return material_cache[CACHE_KEY]
	var result := StandardMaterial3D.new()
	# 本家牌面保留 PBR 受光，使暖象牙色与实体圆角共同形成明暗和高光。
	# 旧的 unshaded 纯白面会把边缘、厚度和入射角差异全部抹平，看起来反而像白色贴片。
...
func _jade_back_material() -> StandardMaterial3D:
	...
	const CACHE_KEY := "body:sichuan_2_6_9_emerald_resin_v1"
	if material_cache.has(CACHE_KEY):
		return material_cache[CACHE_KEY]
	var result := StandardMaterial3D.new()
...
func _ivory_body_material(bright_front: bool = false) -> StandardMaterial3D:
	var cache_key := "body:sichuan_2_6_9_ivory_shell_v1:%s" % ("bright" if bright_front else "standard")
	if material_cache.has(cache_key):
		return material_cache[cache_key]
	var result := StandardMaterial3D.new()
	# 主牌身必须是有厚度的暖象牙壳；绿色只属于独立的 MahjongTileBack 层。
	# 过去把整个主体覆盖成深绿，亮牌只剩一张白色平面，是“扁贴图感”的根因。
	result.albedo_color = Color("E8E1D2") if bright_front else Color("E4DED2")
	result.roughness = 0.28
	result.metallic = 0.01
	result.clearcoat_enabled = true
	result.clearcoat = 0.34
...
func _apply_state_marker(
	...
	var marker_key := "none"
	# 报杠是内江玩法的持续状态，明牌和隐藏牌背都必须保留暖金边框；它的
	# 语义优先于临时的推荐/风险标记。选中仍由实体抬升表达，两种状态可共存。
	if bao_gang_declared:
		marker_color = Color(0.96, 0.64, 0.16, 0.78)
		marker_key = "bao_gang_declared"
	elif selected:
		pass
	elif danger:
