class_name NeijiangTableStage3D
extends Node3D
...
func find_tile_at_screen(screen_position: Vector2) -> int:
	...
	for index in range(self_hand_keys.size() - 1, -1, -1):
		...
		var tile := tile_nodes.get(key) as NeijiangTile3D
...
func set_reduced_motion(enabled: bool) -> void:
	...
	for tile_value in tile_nodes.values():
		var tile := tile_value as NeijiangTile3D
		if tile != null:
...
func _append_hand_entries(
	...
	if seat == 0:
		display_hand = _sort_human_hand_for_display(display_hand)
...
func _apply_entries(desired: Dictionary) -> void:
	...
	for desired_key in desired.keys():
		...
		for existing_key in tile_nodes.keys():
			if not str(existing_key).begins_with("hand_0_"):
				continue
			var candidate := tile_nodes[existing_key] as NeijiangTile3D
	...
	for key in desired.keys():
		...
		if is_new:
			tile = TILE_SCRIPT.new() as NeijiangTile3D
		...
		tile.configure(
			...
			bool(data.get("bao_gang_declared", false)),
...
func _entry(
	...
	return {
		...
		"danger": danger,
		"bao_gang_declared": bao_gang_declared,
		"latest": latest,
		"pickable": pickable,
		"scale": scale_value,
...
func _upright_hand_center_y(seat: int) -> float:
	...
	return TABLETOP_CONTACT_Y \
		+ NeijiangTile3D.TILE_SIZE.z * _hand_scale_for_seat(seat) * 0.5 \
...
func _configure_self_row_layout(hand_count: int, player: Dictionary, has_detached_draw: bool = false) -> void:
	...
	var tile_width := NeijiangTile3D.TILE_SIZE.x * self_layout_scale
...
func _sort_human_hand_for_display(hand: Array) -> Array:
	var result := hand.duplicate(true)
	result.sort_custom(func(a: Dictionary, b: Dictionary) -> bool:
		var suit_order := {"tiao": 0, "tong": 1}
...
func _is_bao_gang_tile(player: Dictionary, tile: Dictionary) -> bool:
	if tile.is_empty():
		return false
	var keys: Array = player.get("bao_gang_tiles", [])
	if keys.is_empty():
		return false
	return keys.has("%s_%d" % [str(tile.get("suit", "")), int(tile.get("rank", 0))])
...
func _build_contract(snapshot: Dictionary, all_hands: Array, players: Array, desired: Dictionary) -> Dictionary:
	...
	return {
		...
		"center_compass_asset": "res://res/art/3d/neijiang_center_compass_v2.glb",
		...
		"self_hand_world_gap": maxf(
			0.0,
			self_layout_pitch - NeijiangTile3D.TILE_SIZE.x * self_layout_scale
		),
		"discard_scale": DISCARD_SCALE,
		"tile_physical_size": NeijiangTile3D.TILE_SIZE,
		"flat_back_cover_size": NeijiangTile3D.CONCEALED_BACK_SIZE,
		...
		"self_hand_lighting": "sichuan_2_6_9_warm_ivory_shell_with_stable_face_carrier",
		...
		"neijiang_two_suit_sort": "rightmost_then_rank_then_tile_id",
