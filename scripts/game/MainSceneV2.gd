...
func _write_neijiang_3d_runtime_probe() -> void:
	...
	var probe := {
		...
		"center_display_shape": str(stage_visual_contract.get("center_display_shape", "")),
		"center_active_color_hex": str(stage_visual_contract.get("center_active_color_hex", "")),
		"center_inactive_color_hex": str(stage_visual_contract.get("center_inactive_color_hex", "")),
...
func _update_v17_player_info_panels(snapshot: Dictionary) -> void:
	...
	for seat in [1, 2, 3]:
		...
		if ding_que_badge != null:
			ding_que_badge.text = _ding_que_short_text(ding_que)
...
func _apply_self_ding_que_style(suit: String = "") -> void:
	var style := StyleBoxFlat.new()
	style.bg_color = _self_ding_que_color(suit).darkened(0.18)
	style.border_color = Color(0.95, 0.86, 0.62, 0.92)
	...
	self_ding_que_label.add_theme_font_size_override("font_size", 19)
	self_ding_que_label.add_theme_color_override("font_color", Color(0.96, 0.94, 0.86, 1.0))
	self_ding_que_label.add_theme_color_override("font_outline_color", Color(0.12, 0.08, 0.05, 0.94))
	self_ding_que_label.add_theme_constant_override("outline_size", 2)
	self_ding_que_label.custom_minimum_size = Vector2(128, 42)
...
func _should_show_ding_que_badges(snapshot: Dictionary) -> bool:
	if not bool(snapshot.get("rules", {}).get("use_ding_que_phase", true)):
		return false
	return not bool(snapshot.get("human_ding_que_pending", false)) and int(snapshot.get("current_phase", 0)) != 2
...
func _build_ai_live_readout_lines(snapshot: Dictionary, latest_turn_snapshot: Dictionary) -> Array[String]:
	...
	lines.append("当前策略：%s｜阶段：%s｜桌面威胁：%d" % [
		str(strategy_profile.get("mode_label", "两门速听")),
